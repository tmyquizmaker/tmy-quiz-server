...
class ManualQuizPage(ctk.CTkFrame):
    ...
    def __init__(self, master, on_quiz_created, back_callback):
        ...
        # ---------------------------------------------
        # 1. BARRE SUPÉRIEURE (Titre du Quiz & Actions)
        # ---------------------------------------------
        ...
        self.quiz_title_entry = ctk.CTkEntry(
            ...
            font=("Arial", 14, "bold"), width=350, fg_color="#121620", border_color="#2B303C"
        )
        self.quiz_title_entry.pack(side="left", padx=10, expand=True, fill="x")
    ...
    def set_correct_answer(self, lettre):
        ...
        for l, btn in self.correct_buttons.items():
            if l == lettre:
                btn.configure(fg_color="#2E7D32") # Vert pour la bonne réponse
            else:
    ...
    def finish_quiz_creation(self):
        """Valide et transmet le quiz terminé"""
        self.save_current_form_data()
        title = self.quiz_title_entry.get().strip() or "Mon Quiz Personnalisé"
        
        # Redirection vers la page de Code PIN
        self.on_quiz_created(quiz_title=title, quiz_data=self.questions)